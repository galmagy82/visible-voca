...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /* 사용량 확인 (extract는 별도 카운트 안 함 — 검색 시에만 카운트) */
    /* 유료 키만 사용 — 텍스트 검색/extract는 한도 초과해도 유료 키로 계속 허용, 이미지만 차단 */
    const apiKey = GEMINI_API_KEY
    if (action === "search" || action === "image") {
      const usage = await checkAndIncrementUsage(userId)
      /* 이미지는 무료 체험 초과 시 차단 (구독자만 이미지 생성 가능) */
      if (action === "image" && !usage.allowed && !usage.subscribed) {
        return new Response(
    ...
    /* extract는 카운트도 차단도 없이 유료 키로 항상 허용 */
