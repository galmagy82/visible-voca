...
Selection rules for "study_items":
  ...
  - Three-word phrasal verbs: when a phrasal verb is habitually followed by a specific preposition that introduces its object, extract ALL THREE words together. The preposition is part of the lexical unit, not a separate object-introducer.
    Examples: "look forward to", "cuddle up with", "stand up for", "come down with", "put up with", "run out of", "get away with", "look up to", "go along with", "catch up with", "make up for", "keep up with", "look out for".
    Test: would an ESL dictionary list this as a single phrase? If yes, keep all three words. If the preposition is interchangeable with many other prepositions (literal direction/motion), drop it.
  - Phrase extraction guard (applies to all multi-word items — phrasal_verb, idiom, collocation): a multi-word phrase is worth extracting ONLY when (1) the combined meaning is non-compositional / idiomatic (e.g. "give up", "hot dog", "red tape", "dark horse"), OR (2) at least one word is clearly above the learner's GE level AND the phrase teaches a useful pattern around it. If all words are at/below the learner's GE level AND the meaning is compositional (= literal sum of parts), DO NOT extract the phrase — instead extract only the single word that has learning value, or skip entirely. Examples: "everything about" → skip (all GE 1, compositional); "crawly things" → extract "crawly" alone (thing is GE 1, compositional); "ten-story building" → extract "ten-story" alone (building is GE 2, compositional); "wooden table" → extract "wooden" alone (table is GE 2, compositional); "give up" → extract as phrasal_verb (non-compositional); "hot dog" → extract as collocation (non-compositional).
...
1. Extract the original English text exactly as it appears on the page, preserving line breaks (use \\n).
   MANDATORY: every printed line in the photo must end with \\n. Treat each visually distinct line as a separate line.
