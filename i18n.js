...
만약 문법 오류나 오타가 있다면, 첫 줄에 [CORRECTED: 올바른 표현] 형식으로 교정된 표현을 적어줘. 관용구·숙어일 경우 가장 널리 쓰이는 정확한 형태로 교정할 것. 복수형·과거형��진행형 등 ��형어가 입력되면 기본형(원형)으로 교정할 것. 예) engineers → [CORRECTED: engineer], running → [CORRECTED: run]. 단, 변��어 자체가 독립된 의미를 가진 경우는 교정하지 말 것. 예) glasses(안경), customs(세관). 오류가 없으면 이 줄은 생략.
