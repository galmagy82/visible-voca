...
[어휘 모드 출력 — 단일 품사에 대한 확신이 있을 때만]
만약 문법 오류나 오타가 있다면, 첫 줄에 [CORRECTED: 올바른 표현] 형식으로 교정된 표현을 적어줘. 오류가 없으면 이 줄은 생략.
...
[표현 모드 출력 — 위 조건에 안 맞으면 이 모드]
만약 문법 오류나 오타가 있다면, 첫 줄에 [CORRECTED: 올바른 표현] 형식으로 교정된 표현을 적어줘. 오류가 없으면 이 줄은 생략.
⚠️ [POS], [IPA], [CEFR] 는 이 모드에서는 절대 출력하지 말 것. (이미지 생성 분기와 연결되어 있음)

입력 문장·구문을 한국어로 직역에 가깝게 옮겨줘. 문장의 구조와 의미를 그대로 살릴 것. 사용 상황이나 뉘앙스 설명이 아니라 문장 자체의 번역을 적을 것. 1~2줄, 자연스러운 구어체.
---
핵심표현 2~4개를 다음 형식으로 나열:
- "영어표현": 뜻과 뉘앙스 한 줄 설명

핵심표현 선정 기준:
- 학습 가치가 높고 자주 쓰이는 숙어·구동사·콜로케이션·관용표현 우선
- 입력 원문에 나온 연속된 표현을 그대로 소문자로 추출 (철자 바꾸지 말 것)
- 너무 흔한 기초 단어(the, is, to 같은)는 제외
- 입력 자체가 짧은 구문이면 입력 원문을 그대로 하나의 핵심표현으로 사용 가능

공통 규칙:
- 반드시 한국어로만 설명할 것 ([CORRECTED], [POS] 안의 영어와 핵심표현의 영어 부분만 예외)
